    ...
    if (ta.level.clipping) {
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: 'Peaks will sit below 0 dBFS — clean, distortion-free output.',
          tradeoff: 'Track will be 3 dB quieter overall; may need to rebalance relative to other tracks.',
    ...
    if (diff > 12) {
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: `Both tracks will be within ${(diff / 2).toFixed(0)} dB of each other — more balanced stereo image.`,
          tradeoff: `"${loudTrack?.name}" will be less prominent; creative intent may require it louder.`,
    ...
    for (const pair of analysis.maskingPairs.slice(0, 3)) {
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: 'Both tracks will sound clearer and more distinct in the mix.',
          tradeoff: `"${nonDominantTrack?.name}" will lose some body in the cut frequency range.`,
    ...
    if (lufs > targetLufs + 3 && lufs > -Infinity) {
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: `Mix will play at intended volume on streaming platforms without gain reduction.`,
          tradeoff: 'Perceived loudness will decrease; competitive loudness may suffer slightly.',
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: `Mix will sit at competitive loudness (${targetLufs} LUFS) for the genre.`,
          tradeoff: 'Raising loudness reduces dynamic range — transients may lose punch.',
          alternatives: ['Adjust per-track levels instead of master', 'Use a different limiter algorithm'],
        },
        timestamp: Date.now(),
      });
    ...
    if (tracksNeedingAdjustment.length > 0) {
      ...
      suggestions.push({
        ...
        explanation: {
          ...
          expectedResult: 'All tracks will peak around -6 dBFS, giving effects chains clean headroom.',
          tradeoff: 'Overall mix level will change — master fader may need adjustment.',
